...
#pragma region Draw Graphical Objects
    ...
    {
        _appSet.GRID_V_DELTA = _appSet.CAMERA_WIDTH / 100;
...
void MainWindow::updateImage(const cv::Mat &frame)
{
    if (frame.empty())
    {
        //qDebug() << "Пустой кадр получен!";
        return;
    }
    cv::Mat workingFrame = frame.clone();
    drawGraphicalObjects(workingFrame);
}
